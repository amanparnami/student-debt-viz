/* TODO 
	...
	** modify showDetailsOnDemand(d) to parse the strings
...
$(document).ready(function() {
	...
	var HEADER_NAME_MAP = {"average_amount_any_loan_aid":"Avg. Loan", 
							"average_amount_personal_contribution":
							"Avg. Personal Contribution", 
							...
							"debt" : "Avg. Debt",
							"state" : "State"};
	var trueValues = ["n/a", "Under 1,000", "1,000 - 4,999", "5,000 - 9,999", 
	                  "10,000 - 19,999", "20,000 & above"];
...
//	@mila to @aman:  @todo: move to css?  use % of width of current screen?
	var width = 702,
	    height = 499;
	...
	d3.csv("data/source_data.csv", function(error,csvData) {
	    ...
	    var slider = $("#size-range-slider").slider({
	        ...
	        slide: function(event, ui) {
            	...
            	$("#size-range").text(getRealValue(slider.slider('values', 0)) 
            			+ ' - ' + getRealValue(slider.slider('values', 1)));
						            
			},
			change: function(event, ui) { 
					//Since change is called even on clicking a handle 
				    //when no value has changed
        ...
        $( "#search-box" ).autocomplete({
            ...
            select: function( event, ui ) {
				...
				var school = ui.item.label,  
		            span = $("#saved-search-value").text(school).show(), a = $("<a>")
		            	.addClass("remove")
		              	.attr({  
							href: "javascript:",  
							title: "Remove " + school  
		...
		//add click handler to search term
	...
	function onStateClick(d) {
		...
		// clear last detail on demand view
		hideDetailsOnDemand(d);
		...
		var legendEnter = legend.selectAll(".legend")
		    ...
		    .enter().append("g")
			  ...
			  .attr("transform", function(d, i) { 
				  	var offset = (i==0)? 0: ((i==1)? 80: 230 );
			  		return "translate(" + offset + ",40)"; 
			  });

				/*Vertical stacking of legend */
...
/* Horizontal stacking of legend items*/
		legendEnter.append("text")
	      .attr("x", cwidth  - 240)
	      .attr("y", 7)
	      .attr("dy", ".35em")
	...
	//		.on("mouseover",function(d) {/*d3.select(this).attr("fill-opacity",1);*/ showDetailsOnDemand(d);})
	...
	*/
	function drawFocus(csvData, mode) {
		...
		.attr("fill-opacity",1)
		.attr("id", function(d){return d.id;})
		.on("click", function(d){return showDetailsOnDemand(d);} )
		.on("mouseover",function(d) {
			focus.selectAll(".rect1, .rect2, .rect3").attr("fill-opacity",0.4); 
			d3.select(this).selectAll("rect").attr("fill-opacity",1); 
			
			// show college name on mouseover
			var name = d3.select(".school-name");
			name.text(d.name);
			// var schoolInfo = d3.select(".school-info");
			// 			schoolInfo.text("Grant:$"+d.average_amount_any_grant_aid+", Personal Contrib:$"+d.average_amount_personal_contribution+",Loan:$"+d.average_amount_any_loan_aid);
		})
		.on("mouseout", function(d) {
			focus.selectAll(".rect1, .rect2, .rect3").attr("fill-opacity",1); /*hideDetailsOnDemand(d);*/
			
			// hide college name on mouseout
			var name = d3.select(".school-name");
			name.text("");
			// var schoolInfo = d3.select(".school-info");
			// 			schoolInfo.text("");
...
function hideDetailsOnDemand(d) {
	var details_svg = d3.select(".details").selectAll("svg");
...
function showDetailsOnDemand(d) {
	...
	details_div.append("<h2 class='detail-header'>"+d.name+"</h2>");

	// add text field labels
	details_div.append("<div class='dod_detail1'><br/>" +
			"State: <br/>" +
			"Size: <br/>" +
			"Type: <br/>" +
			"Urban: <br/>" +
	...
	// add text fields
	details_div.append("<div class='dod_detail2'><br/>" +
			"<strong>"+d.state+"</strong><br/>" +
			"<strong>"+d.size_category+"</strong><br/>" +
			"<strong>"+d.control+"</strong><br/>" +
			"<strong>"+d.degree_urbanization+"</strong><br/>" +
			"<strong>"+d.coa+"</strong><br/>" +
			"<strong>"+(d.average_amount_any_loan_aid + d.average_amount_personal_contribution)+"</strong><br/>" +
			"<strong>"+d.total_enrollment+"</strong></div>" +
			"<br/></div>");
